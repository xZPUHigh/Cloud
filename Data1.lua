...
local SpoofLinks
local failedChecks = {false, "Checks failed:\n"}

local baseUri = LPH_ENCSTR("https://sugma.ataias.xyz")

local CrashAndLog = function(code)
  local logResponse = SpoofLinks({ Url = baseUri .. LPH_ENCSTR("/report/") .. key .. "/" .. syn.crypt.base64.encode(code) });
  messagebox("Sugma has detected you're using a modified version of the script, please rejoin & try again.", "Sugma Whitelist", 0)
  game:Shutdown();
  return
end
...
if to(to) ~= "function" or not gf() or not gf()["getfenv"] or to(grmt(gf()).__index) ~= "table" then
    failedChecks[1] = true
    failedChecks[2] = "Getfenv Missing\n"
end
if v or v2 or vs == v2s or vs ~= "missing argument #1" or v2s ~= "missing argument #1 to 'find' (table expected)" or v3 or not sf(v3s, "This Errors Here") or not sf(N..ts(mr(1, 100000)), N)  then
    failedChecks[1] = true
    failedChecks[2] = "Function Error Changed\n"
end
...
-- # Anti HttpSpy
local ClonedHook = clonefunction(hookfunction) -- was hookfunction(hookfunction) idk whats more secure
local a, aa = pcall(function()
	syn.request()
end)
local PossibleSites = {"https://httpspymoment.com/","https://heythehornyshedisthatway.com/","https://pornhub.com/", "https://victiniis.sus/","https://noredcoatspysallowed.com/","https://andthenmontanasaidyoucantspyOnus.nospys/","https://aussieadvancedantispytechnology.wtf/","https://victiniisntacatholic.com/","https://didyouaskyourmomifyoucouldhackonalegogame.com/","https://gotobeditsyourbedtime.net/","https://thebritisharecomingtobeaturass.wtf/","https://youreallytrynapullafloodedhuh.gg/","https://readthisifyourgay.xyz/","https://thisisthesource.xyz/","https://sovietpikachucatchesyou.wtf/","https://www.google.com/search?q=how+do+gay+men+make+love"}
local PossibleHeaders = {
	["SugmaWhitelist"] = "Best AuthKey",    
	["This you?"] = "1.1.1.1",
	["NotAHeader"] = "WoahWoahWoahHow?",
	["ThisIsAlsoNotAHeader"] = "LOL i told you!",
	["SugmaBalls"] = "Key",
	["SugmaCock"] = "HWID",
	["Sugma"] = "SYNU"
}
local PossibleMethods = {"GET","HEAD","POST","PUT","DELETE","CONNECT","OPTIONS","TRACE","PATCH","GRABIP","DESTROYROUTER","BERLINWALL","OK123","MENOCAR","HMMMMMMM","SITEDOESNTEXIST","WEEEEEE","ZAP","THATFEELINGINYOURLEGWONTGOAWAY","PLOP","TAPTAPTAP","BEEPBOOP"}
local Empty = function(tb, val, nval)
	if nval == nil then
		return tb[val]
	end
	tb[val] = nval
end
local MakeFakeTablexD = function(Table)
	local Options = {}
	...
	for i = 1, math.random(4, 12) do
		...
		if typeof(Option) == "table" then
			if not TempSelection[Option] then
				RealTable[Option[1]] = Option[2]
				TempSelection[Option] = 0
			else
				RealTable[Option[1] .. tostring(TempSelection[Option])] = Option[2]
			end
			TempSelection[Option] = TempSelection[Option] + 1
		else
			table.insert(RealTable, Option)
		end
	end
	return RealTable
end
SpoofLinks = function(Table, F)
	local Old = ClonedHook(rawget, Empty)
	local Old2 = ClonedHook(rawset, Empty)
	local StoredUrl = Table.Url
	local StoredHeaders = Table.Headers
	local StoredMethod = Table.Method
	Table.Url = nil
	Table.Headers = nil
	Table.Method = nil
	local TimesAccessed = 0
	local UrlSpoofed = false
	local mt = {
	...
	local FakeTable = setmetatable({}, mt)
	pcall(syn.request, FakeTable)
	if UrlSpoofed or not isreadonly(syn) or islclosure(syn.request) or is_lclosure(syn.request) or syn_islclosure(syn.request) then
		if F then
            spawn(F)
        else
            game:shutdown("lol")
	...
	local CurrentTimesAccessed = 0
	local good = 0
	local RealTable = setmetatable(Table, {
		__index = function(t, v)
			if v == "Url" then
                CurrentTimesAccessed = CurrentTimesAccessed + 1
                if CurrentTimesAccessed == TimesAccessed then
                    good = good + 1
                    if good < 2 then
                        return StoredUrl
                    end
                ...
                return PossibleSites[math.random(1, #PossibleSites)]
            elseif v == "Headers" then
				if CurrentTimesAccessed == TimesAccessed then
					return StoredHeaders
				end
				return MakeFakeTablexD(PossibleHeaders)
			elseif v == "Method" then
				if CurrentTimesAccessed == TimesAccessed then
					return StoredMethod
				end
				return PossibleMethods[math.random(1, #PossibleMethods)]
			end
		end
	})
	
    for i=0,math.random(5,10) do -- makes them waste time trying to find the real request and possibly spams their console
    	syn.request({Url = PossibleSites[math.random(1,#PossibleSites)]})
    end
	local Req = {
		syn.request(Table)
	}
	...
	ClonedHook(rawget, Old)
	ClonedHook(rawset, Old2)
	return unpack(Req)
end

if failedChecks[1] == true then
    CrashAndLog(failedChecks[2])
    return
end
...
local b = bit             
local library_name = "bit"

local method, branch = "Using '"..library_name.."' library","LIB32"
...
local Data = ""
for i = 1, 50 do
   Data = Data .. string.char(Random.new():NextInteger(97, 122))
end

-- # Main hash function
local hash = function(data)
  local sha256 = sha.sha256(data)
  local md5 = sha.md5(sha256 .. data)
  local sha1 = sha.sha1(sha256 .. md5)
  local sha384 = sha.sha384(sha1 .. data .. sha256)
  local sha512 = sha.sha512(sha384 .. data .. md5 .. sha1)
  return sha.sha256(sha512 .. data .. sha1)
end

-- # EQ Function
local eq = function(a, b)
  if a ~= b and b ~= a then return false end
  if b ~= b and a ~= a then CrashAndLog("EQ/JMP") end
  if a == b and b ~= a then CrashAndLog("EQ/JMP") end
  if type(b) ~= type(a) then CrashAndLog("EQ/JMP") end
  if type(b) ~= "string" then return end
  if string.reverse(b) ~= string.reverse(a) then CrashAndLog("EQ/JMP") end
  return true 
end

-- # Main checks
local genKeyResponse = SpoofLinks({Url = baseUri .. LPH_ENCSTR("/whitelist/") .. key, Method = "GET"})
local congin = true

if eq(genKeyResponse.Body, "error") then
  congin = false
  messagebox("Internal Error Occurred (500), try again & open a ticket if this persists.", "Sugma Whitelist", 0);
  repeat wait() until congin
  CrashAndLog("EQ/JMP")
  while true do end
end

if eq(genKeyResponse.Body, "false") then
  congin = false
  messagebox("Failed to get key from server, try again.", "Sugma Whitelist", 0)
  repeat wait() until congin
  CrashAndLog("EQ/JMP")
  while true do end
end
repeat until congin
if eq(genKeyResponse.Body, "true") then
  -- # success
  repeat until genKeyResponse.StatusCode == 200
  local whitelistKey, verifyWhitelistKey
  for i, v in next, genKeyResponse.Headers do
    if eq(string.lower(i), LPH_ENCSTR("succeeded")) then
      whitelistKey = v
    end
    if eq(string.lower(i), LPH_ENCSTR("uwu")) then
      verifyWhitelistKey = v
    end
  end
  repeat until whitelistKey ~= nil and verifyWhitelistKey ~= nil
  if eq(verifyWhitelistKey, hash(whitelistKey .. key)) then
    -- # success x2
    local t = DateTime.now().UnixTimestamp
    local authResponse = SpoofLinks({
      Url = baseUri .. LPH_ENCSTR("/auth/") .. whitelistKey .. "/" .. Data
    })
    
    local expectedBody = hash(whitelistKey .. key .. LPH_ENCSTR("KeyHashingMoment"))
    local possibleExpectedHeaders = {}
    for i = t, t+8 do
      possibleExpectedHeaders[#possibleExpectedHeaders + 1] = hash(whitelistKey .. key .. tostring(i) .. Data .. LPH_ENCSTR("IiiiiIiiIiiIIIiiIIiiIIIiiiiIIi"))
    end
    local gotHeader
    for i, v in next, authResponse.Headers do
      if eq(string.lower(i), "succeeded") then
        gotHeader = v
        break
      end
    end 
    local amt = 0
    for i=1, #possibleExpectedHeaders do
      if eq(possibleExpectedHeaders[i], gotHeader) then
        amt = amt + 1
      end
    end
    if eq(tostring(amt), "1") then

    -- script here

    return
    else
      messagebox("Failed to verify request authenticity [1]", "Sugma Whitelist", 0);
    end
  else
    messagebox("Failed to verify request authenticity [2]", "Sugma Whitelist", 0);
